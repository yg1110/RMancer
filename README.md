...
데이터 기반 개인 맞춤 운동 프로그램 추천 서비스

RMancer는 인바디 분석과 운동 목표를 바탕으로 사용자에게 최적화된 운동 루틴을 추천하는 웹 애플리케이션입니다. 사용자의 개인 정보, 목표, 경험 수준, 인바디 데이터, 1RM 기록을 종합 분석하여 맞춤형 운동 프로그램을 제공합니다.
...
RMancer는 풀스택 웹 애플리케이션으로, 사용자가 효과적이고 체계적으로 운동을 할 수 있도록 돕는 서비스입니다.
...
- **원자성 보장**: Prisma 트랜잭션을 활용한 다중 데이터 일괄 처리
- **데이터 일관성**: 여러 테이블에 걸친 작업을 단일 트랜잭션으로 처리하여 부분 실패 방지
- **롤백 지원**: 작업 중 하나라도 실패하면 모든 변경사항이 자동 롤백되어 데이터 무결성 보장
- **대시보드 프로필 생성**: 목표 프로필, 인바디 기록, 1RM 기록을 트랜잭션으로 안전하게 일괄 생성
...
- 목표 설정 및 수정
- **트랜잭션 기반 프로필 일괄 생성**: 목표 프로필, 인바디 기록, 1RM 기록을 단일 트랜잭션으로 안전하게 생성
  - 모든 데이터가 성공적으로 저장되거나 모두 롤백되는 원자성 보장
  - 부분 실패로 인한 데이터 불일치 방지
- 운동 프로그램 추천으로 연결
